...
export const FeaturesCarousel = () => {
  ...
  return <div className="relative h-[180px] sm:h-[220px] w-full overflow-hidden rounded-xl shadow-2xl" onMouseEnter={() => setIsAutoPlaying(false)} onMouseLeave={() => setIsAutoPlaying(true)}>
      ...
      <div className="relative z-10 flex h-full items-center px-4 sm:px-6">
        <div className="max-w-lg text-white">
          ...
          <p className="text-sm sm:text-base text-gray-100 drop-shadow-md animate-fade-in-up" style={{
          animationDelay: '0.2s'
        }}>
      ...
      <Button variant="ghost" size="icon" className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 z-20 bg-black/40 hover:bg-black/60 text-white h-8 w-8 sm:h-10 sm:w-10 backdrop-blur-sm border border-white/20 hover:border-white/40 transition-all duration-300" onClick={prevSlide}>
        <ChevronLeft className="h-4 w-4 sm:h-5 sm:w-5" />
      </Button>
      
      <Button variant="ghost" size="icon" className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 z-20 bg-black/40 hover:bg-black/60 text-white h-8 w-8 sm:h-10 sm:w-10 backdrop-blur-sm border border-white/20 hover:border-white/40 transition-all duration-300" onClick={nextSlide}>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-black/30">
        <div className="h-full bg-gradient-to-r from-red-500 to-red-600 transition-all duration-300" style={{
