...
export const FeaturesCarousel = () => {
  ...
  return (
    <div className="relative h-[200px] sm:h-[240px] w-full overflow-hidden rounded-2xl shadow-legal card-depth-3" 
         onMouseEnter={() => setIsAutoPlaying(false)} 
         onMouseLeave={() => setIsAutoPlaying(true)}>
      {/* Background Image with enhanced overlay */}
      <div className="absolute inset-0 bg-cover bg-center transition-all duration-1000 transform" 
           style={{
        ...
        {carouselData.map((_, index) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-all duration-300 ${
              index === currentSlide 
                ? 'bg-accent-legal shadow-lg shadow-accent-legal/50' 
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-background-deep/50">
        <div className="h-full bg-gradient-to-r from-accent-legal via-primary to-accent-legal transition-all duration-500 shadow-lg shadow-accent-legal/30" 
             style={{ width: `${(currentSlide + 1) / carouselData.length * 100}%` }} />
      </div>

      {/* Subtle particle effect overlay */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-4 right-8 w-2 h-2 bg-accent-legal/40 rounded-full animate-legal-pulse" 
             style={{ animationDelay: '0s' }} />
        <div className="absolute top-12 right-16 w-1 h-1 bg-white/30 rounded-full animate-legal-pulse" 
             style={{ animationDelay: '1s' }} />
        <div className="absolute top-8 right-12 w-1.5 h-1.5 bg-accent-legal/30 rounded-full animate-legal-pulse" 
