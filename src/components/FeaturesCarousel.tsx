    ...
    >
      ...
      <div className="relative z-10 flex h-full items-center px-6 sm:px-8">
        <div className="max-w-2xl text-white">
          ...
          <p className="text-sm sm:text-base md:text-lg text-gray-100 drop-shadow-lg animate-fade-in-up leading-relaxed" style={{ animationDelay: '0.2s' }}>
            {currentItem.description}
          </p>
          
          {/* Professional accent line */}
          <div className="mt-4 w-20 h-1 bg-gradient-to-r from-accent-legal to-accent-legal/60 rounded-full animate-fade-in-up" style={{ animationDelay: '0.4s' }} />
        </div>
      </div>
      ...
      <Button 
        variant="ghost" 
        size="icon" 
        className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 z-20 glass-effect-legal hover:bg-white/20 text-white h-10 w-10 sm:h-12 sm:w-12 border border-white/30 hover:border-accent-legal/50 transition-all duration-300 hover-glow-legal" 
        onClick={prevSlide}
      >
      ...
      <Button 
        variant="ghost" 
        size="icon" 
        className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 z-20 glass-effect-legal hover:bg-white/20 text-white h-10 w-10 sm:h-12 sm:w-12 border border-white/30 hover:border-accent-legal/50 transition-all duration-300 hover-glow-legal" 
        onClick={nextSlide}
      >
        ...
        {carouselData.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full transition-all duration-300 ${
              currentSlide === index 
                ? 'bg-accent-legal scale-125 shadow-lg shadow-accent-legal/50' 
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-4 right-8 w-2 h-2 bg-accent-legal/40 rounded-full animate-legal-pulse" style={{ animationDelay: '0s' }} />
        <div className="absolute top-12 right-16 w-1 h-1 bg-white/30 rounded-full animate-legal-pulse" style={{ animationDelay: '1s' }} />
        <div className="absolute top-8 right-12 w-1.5 h-1.5 bg-accent-legal/30 rounded-full animate-legal-pulse" style={{ animationDelay: '2s' }} />
      </div>
    </div>
  );
};
