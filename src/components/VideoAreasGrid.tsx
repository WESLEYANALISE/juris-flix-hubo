...
export const VideoAreasGrid = () => {
  ...
  } = useYouTube(selectedPlaylistUrl);
  ...
  return <div className="max-w-7xl mx-auto p-4 sm:p-6 md:p-8 px-[10px] py-[24px]">
      {/* Header com navegação */}
      <div className="flex items-center gap-4 mb-6">
        {selectedArea && <Button variant="outline" onClick={handleBack} size="sm" className="bg-primary/10 border-primary/30 text-primary hover:bg-primary/20 hover:border-primary/50 font-medium">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">
            {selectedArea ? `${videoAreas[selectedArea]?.videos.length || 0} playlists disponíveis` : 'Escolha uma área para começar seus estudos'}
          </p>
        </div>
      </div>
      ...
      <div className="relative max-w-md mb-8">
        ...
        <Input placeholder={selectedArea ? "Buscar playlist..." : "Buscar área de estudo..."} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
      </div>

      {!selectedArea ?
    // Grid de áreas com visual de videoaulas
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {Object.entries(filteredAreas).map(([area, data]) => <Card key={area} className="group hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden border-2 hover:border-accent-legal/30 bg-gradient-to-br from-slate-900 to-slate-800" onClick={() => handleAreaSelect(area)}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {videoAreas[selectedArea]?.videos.filter(video => !searchTerm || video.nome.toLowerCase().includes(searchTerm.toLowerCase())).map(video => <Card key={video.id} className="group hover:shadow-xl transition-all duration-300 cursor-pointer overflow-hidden" onClick={() => handleVideoSelect(video)}>
      ...
      {(selectedArea && videoAreas[selectedArea]?.videos.filter(video => !searchTerm || video.nome.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 || !selectedArea && Object.keys(filteredAreas).length === 0) && <Card className="text-center p-8">
